...
/// Create a CVPixelBuffer (kCVPixelFormatType_32BGRA) from uint8 RGBA
/// interleaved pixel data (R,G,B,A,R,G,B,A,…). The data is copied.
static CVPixelBufferRef create_pixel_buffer_from_rgba(uint8_t* rgba, int width, int height) {
    NSDictionary* attrs = @{
    ...
    for (int row = 0; row < height; row++) {
        for (int col = 0; col < width; col++) {
            ...
            size_t dstIdx = row * bytesPerRow + col * 4;
            // BGRA pixel: B=src[2], G=src[1], R=src[0], A=255
            dest[dstIdx]     = rgba[srcIdx + 2];
            dest[dstIdx + 1] = rgba[srcIdx + 1];
            dest[dstIdx + 2] = rgba[srcIdx];
            dest[dstIdx + 3] = 255;
        }
    }
...
CoreMLImageInput coreml_image_input_create(int width, int height, void* rgbaData, CoreMLError* error) {
    CVPixelBufferRef pb = create_pixel_buffer_from_rgba((uint8_t*)rgbaData, width, height);
    if (pb == NULL) {
        set_error(error, 1, nil);
        return NULL;
    }
    return (void*)pb;
}
...
void coreml_image_input_free(CoreMLImageInput input) {
    if (input != NULL) {
        CVPixelBufferRelease((CVPixelBufferRef)input);
    }
}
...
bool coreml_model_predict_with_images(CoreMLModel model,
    ...
    @autoreleasepool {
        ...
        // Build input dictionary with CVPixelBuffer-backed feature values
        NSMutableDictionary<NSString*, MLFeatureValue*>* inputDict = [NSMutableDictionary dictionary];
        for (int i = 0; i < num_inputs; i++) {
            NSString* name = [NSString stringWithUTF8String:input_names[i]];
            CVPixelBufferRef pb = (CVPixelBufferRef)images[i];
            MLFeatureValue* value = [MLFeatureValue featureValueWithPixelBuffer:pb];
            inputDict[name] = value;
        }
        ...
        // Extract outputs (same as coreml_model_predict)
        for (int i = 0; i < num_outputs; i++) {
            NSString* name = [NSString stringWithUTF8String:output_names[i]];
            MLFeatureValue* value = [result featureValueForName:name];
            if (value == nil || value.multiArrayValue == nil) {
